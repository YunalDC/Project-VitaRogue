...
export default function CoachSignInScreen({ navigation }) {
  ...
  const onCoachSignIn = async () => {
    ...
    try {
      ...
      const existingUser = usnap.exists() ? usnap.data() : {};

      const cref = doc(db, "coaches", user.uid);
      const csnap = await getDoc(cref);
      const existingCoach = csnap.exists() ? csnap.data() : null;

      const hasCoachSignals = (coachData) => {
      ...
      const isCoachAccount =
        existingUser?.coachAccountStatus === "active" ||
        hasCoachSignals(existingCoach);

      if (!isCoachAccount) {
        await firebaseAuth.signOut();
        Alert.alert(
          "Not a coach account",
          "This login is reserved for approved coaches. Please sign in from the customer screen instead."
        );
        return;
      }
      ...
      if (!usnap.exists()) {
        await setDoc(
          uref,
          {
            ...
            email: user.email,
            coachProfileComplete: existingCoach?.profileComplete ?? false,
            coachEmailVerified: existingUser?.coachEmailVerified ?? true,
