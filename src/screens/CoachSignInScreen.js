...
export default function CoachSignInScreen({ navigation }) {
  ...
  const onCoachSignIn = async () => {
    ...
    try {
      ...
      // Get both user and coach data simultaneously
      ...
      const isCoach = userData.role === "coach" || userData.coachAccountStatus === "active" || !!coachData;

      if (!isCoach) {
        await firebaseAuth.signOut();
        Alert.alert(
          "Not a coach account",
          "This login is reserved for coaches. Please sign in from the customer screen instead."
        );
        return;
      }
      ...
      await setDoc(uref, {
        ...
        coachProfileComplete: coachData?.profileComplete ?? false,
      }, { merge: true });

      // Update or create coach document
      await setDoc(cref, {
        email: user.email,
        name: coachData?.name || user.displayName || userData.displayName || "Coach",
        lastLoginAt: serverTimestamp(),
        ...(coachData ? {} : { // Only set these fields if it's a new coach document
      ...
      // Navigation will be handled by App.js route state
    } catch (e) {
      console.error("Coach Sign In Failed:", e);
